...
Join @arch_supports For Updates
[Source Code](https://github.com/r0ld3x/terabox-downloader-bot) """
    check_if = await is_user_on_chat(bot, "@arch_supports", m.peer_id)
    if not check_if:
        return await m.reply("Please join @arch_supports then send me the link again.")
    check_if = await is_user_on_chat(bot, "@archbots", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @archbots then send me the link again."
        )
...
async def handle_message(m: Message):
    ...
    if int(data["sizebytes"]) > 1024288000 and m.sender_id not in [1317173146]:
        return await hm.edit(
